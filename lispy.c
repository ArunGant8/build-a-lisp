...
#else
// EDITLINE library: for being able to edit the prompt
...
/* Create a pointer to the new Number type lval */
...
/* Create a pointer to the new Error type lval */
lval* lval_err(char* fmt, ...) {
    ...
    /* Allocate space */
    v->err = malloc(ERROR_MSG_SIZE);

    /* printf the error string with a maximum of ERROR_MSG_SIZE - 1
     * characters */
    vsnprintf(v->err, ERROR_MSG_SIZE-1, fmt, va);
...
void lval_del(lval* v) {
    ...
    switch(v->type) {
        ...
        /* For Err or Sym or String free the string data */
...
lval* lval_read_num(mpc_ast_t* t) {
    ...
    return errno != ERANGE ?
        lval_num(x) : lval_err("invalid number"); // Number too long
                                                  // would be a better
                                                  // error message imo
}
...
void lval_print(lval* v); // forward declaration so that the following
                          // code works
...
lval* builtin_error(lenv* e, lval* a) {
    ...
    /* Construt Error from first argument */
...
/* Environment Handling using lenv */
struct lenv {
    lenv* par;
    int count;
    char** syms;
...
lval* lval_eval(lenv* e, lval* v); // This is required for
                          // builtin_eval and also
                          // for lval_eval_sexpr
...
lval* builtin_op(lenv* e, lval* a, char* op) {
    ...
    /* Ensure that all the arguments are numbers */
    for (int i = 0; i < a->count; i++) {
...
lval* builtin(lenv* e, lval* a, char* func) {
    ...
    if (strcmp("\\", func) == 0) { return builtin_lambda(e, a); } 
    lval_del(a);
...
lval* lval_call(lenv* e, lval* f, lval* a); // forward declaration
...
lval* lval_call(lenv* e, lval* f, lval* a) {
    /* We implement automatic currying */
...
/** Function to load the contents of a file and evaluate it */
lval* builtin_load(lenv* e, lval* a) {
...
int main(int argc, char** argv) {
    ...
    /* Create environment before the prompt */
    lenv* e = lenv_new();
    lenv_add_builtins(e);
